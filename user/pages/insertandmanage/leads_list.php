...
<p id="success"></p>
	<div class="table-wrapper">
		...
		<div class="table-responsive table-wrapper">
			...
			<!--<input type="text" id="clientFilter" onkeyup="filterTable()" placeholder="Search for clients..">-->
			...
			<!--<select class="form-control" id="client" name="client">
                ...
                if (mysqli_num_rows($result) > 0) {
                    while($row = mysqli_fetch_assoc($result)) {
                        echo "<option value=" . $row["clientID"]. ">" . $row["company"]. "</option><br>";
                    }
                } else {
			...
			<!--<script>
    $(document).ready(function () {
        // Add change event listener to the client dropdown
        $('#client').on('change', function () {
            // Get the selected client ID
            var selectedClientId = $(this).val();

            // Show/hide table rows based on the selected client ID
            filterTableByClient(selectedClientId);
        });

        // Function to filter the table rows based on the selected client ID
        function filterTableByClient(clientId) {
            var $table = $('#leads-table');
            var $rows = $table.find('tbody tr');
            ...
            if (clientId !== '0') {
                // If a specific client is selected, hide rows that don't match the client ID
                $rows.filter(':not([data-clientid="' + clientId + '"])').hide();
            }
        }
    });
</script>-->
